...
class DijkstraGrader : Runnable {
    ...
    fun judge(
        ...
        feedback +=
            appendFeedback(i, "Running Dijkstra's algorithm from (${start.getX()}, ${start.getY()}) to (${end.getX()}, ${end.getY()})")
        val path = result.dijkstra(start, end)

        // Correct if paths are the same length and have the same elements
        if (path == null) {
            if (corr.path == null) {
                feedback += "PASSED."
                correct++
            ...
            } else if (path.size != correctPath.size || !path.containsAll(correctPath)) {
